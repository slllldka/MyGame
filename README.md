...
### 개발 동기
평소 게임, 특히 메이플스토리를 즐겨했고, 게임 개발에 도전해보기 위함
...
# 4. 문제해결
#### 캐릭터 이동, 몬스터, 아이템, 데미지 출력 등 많은 요소에서 멀티스레드를 활용하다보니 프리징 현상이 많이 일어남<br>

→ 스레드 풀링을 사용하여 개선할 수 있었음<br>

#### 게임을 끄면 진행했던 사항이 모두 초기화되는 문제<br>

→ 로컬 DB(Oracle)을 사용하여 게임 진행 사항들을 모두 저장하고,<br>
게임을 다시 실행했을 때 DB에 저장된 내용들을 불러오도록 구현하여 해결함<br>

# 5. 아쉬웠던 점
스레드를 너무 남용했고, 그에 따라 Lock을 활용한 동시성 제어에 어려움을 겪었음<br>
캐릭터, 몬스터의 움직임에 물리 법칙을 적용하지 못한 것이 아쉬웠음
